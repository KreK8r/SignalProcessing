...
def main():
    ...
    name = 'YbTi3Sb4-1'
    td_filepath = 'td.csv'
    output_dir = '/root/research/publications/YbTi3Bi4-1/fft_temperatures/'
    os.makedirs(output_dir, exist_ok=True)

    # Load data
    data_td = load_data(td_filepath)

    # Extract x and y data
    x_td, y_td = extract_columns(data_td)
    
    
     # Run full analysis on temperature data
    for num, item in enumerate(temperatures):
        x, y = full_analysis_polynomial(x_td[:, num].tolist(), y_td[:, num].tolist())
